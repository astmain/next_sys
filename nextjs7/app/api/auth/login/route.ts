...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!phone || !password) return NextResponse.json({ success: false, message: '手机号和密码不能为空' })
    const user = await prisma.tb_user.findUnique({ where: { phone }, include: { roles: true } })
    if (!user) return NextResponse.json({ success: false, message: '用户不存在' })
    if (user.password !== password) return NextResponse.json({ success: false, message: '密码错误' })
