...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!phone || !password || !name) {
      return NextResponse.json({ success: false, message: '手机号、密码和昵称不能为空' })
    }

    // 检查手机号是否已存在
    const existing_user = await prisma.tb_user.findUnique({
      where: { phone }
    })

    if (existing_user) {
      return NextResponse.json({ success: false, message: '手机号已被注册' })
    }
